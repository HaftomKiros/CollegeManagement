...
namespace CollegeManagementWPF.Views
{
    public partial class DepartmentsPage : Page
    {
        ...
        public DepartmentsPage() { InitializeComponent(); ThemeManager.ThemeChanged += ApplyTheme; ApplyTheme(); ApplyPermissions(); Loaded += async (s,e) => await Load(Q); }
        ...
        private void ApplyPermissions() {
            ...
            Grid1.Visibility     = SessionUser.Has("dept_view")   ? Visibility.Visible : Visibility.Collapsed;
            BtnSave.Visibility   = SessionUser.Has("dept_add")    ? Visibility.Visible : Visibility.Collapsed;
            BtnUpdate.Visibility = SessionUser.Has("dept_update") ? Visibility.Visible : Visibility.Collapsed;
            BtnDelete.Visibility = SessionUser.Has("dept_delete") ? Visibility.Visible : Visibility.Collapsed;
            BtnClear.Visibility  = (SessionUser.Has("dept_add") || SessionUser.Has("dept_update")) ? Visibility.Visible : Visibility.Collapsed;
        }
        ...
        private async void BtnSave_Click(object s, RoutedEventArgs e) {
            ...
            try {
                string did=TxtDeptID.Text.Trim(), dname=TxtDeptName.Text.Trim(), prog=TxtProgram.Text.Trim(), head=TxtHead.Text.Trim();
                bool dup = await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("SELECT COUNT(*) FROM ecc_dof_wukrostmarycollege.departments WHERE dept_id=@k",c); cmd.Parameters.AddWithValue("@k",did); int n=Convert.ToInt32(cmd.ExecuteScalar()); c.Close(); return n>0; });
                ...
                await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("INSERT INTO ecc_dof_wukrostmarycollege.departments (dept_id,dept_name,dept_program,dept_head) VALUES(@d,@n,@p,@h)",c);
                    cmd.Parameters.AddWithValue("@d",did); cmd.Parameters.AddWithValue("@n",dname);
                    cmd.Parameters.AddWithValue("@p",prog); cmd.Parameters.AddWithValue("@h",head);
        ...
        private async void BtnUpdate_Click(object s, RoutedEventArgs e) {
            ...
            try {
                string key=_selKey, dn=TxtDeptName.Text.Trim(), pr=TxtProgram.Text.Trim(), hd=TxtHead.Text.Trim();
                await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("UPDATE ecc_dof_wukrostmarycollege.departments SET dept_name=@n,dept_program=@p,dept_head=@h WHERE dept_id=@key",c); cmd.Parameters.AddWithValue("@n",dn); cmd.Parameters.AddWithValue("@p",pr); cmd.Parameters.AddWithValue("@h",hd); cmd.Parameters.AddWithValue("@key",key); cmd.ExecuteNonQuery(); c.Close(); });
        ...
        private async void TxtFilter_Changed(object s, TextChangedEventArgs e) { string t=TxtFilter.Text.Trim(); await Load(string.IsNullOrEmpty(t)?Q:Q+$" WHERE dept_id LIKE '%{t}%' OR dept_name LIKE '%{t}%'"); }
        ...
        private async void BtnExportPdf_Click(object s, RoutedEventArgs e) => await ExportPdf(new[]{"dept_id","dept_name","dept_program","dept_head"}, new[]{"Dept ID","Name","Program","Head"}, "Departments");
        private async void BtnExportExcel_Click(object s, RoutedEventArgs e) => await ExportExcel(new[]{"dept_id","dept_name","dept_program","dept_head"}, new[]{"Dept ID","Name","Program","Head"}, "Departments");
        ...
        private async Task ExportPdf(string[] fields, string[] headers, string title)
        {
            ...
            try {
                ...
                await Task.Run(() => {
                    ...
                    var sec=doc.AddSection(); sec.PageSetup.Orientation=MigraDoc.DocumentObjectModel.Orientation.Landscape; sec.PageSetup.PageFormat=MigraDoc.DocumentObjectModel.PageFormat.A4;
                    ...
                    var sub=sec.AddParagraph(title+" List"); sub.Format.Font.Size=11; sub.Format.Alignment=MigraDoc.DocumentObjectModel.ParagraphAlignment.Center;
                    sec.AddParagraph($"Generated: {DateTime.Now:dd MMM yyyy  HH:mm}").Format.Font.Size=8;
                    var tbl=sec.AddTable(); tbl.Borders.Width=0.25; tbl.Borders.Color=MigraDoc.DocumentObjectModel.Colors.LightGray;
                    foreach (var _ in headers) tbl.AddColumn("4cm");
