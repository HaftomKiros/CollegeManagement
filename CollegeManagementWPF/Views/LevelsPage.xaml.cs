...
namespace CollegeManagementWPF.Views
{
    public partial class LevelsPage : Page
    {
        ...
        {
            try {
                var list = await Task.Run(() => { var r=new System.Collections.Generic.List<string>(); var c=_db.GetConnection(); c.Open(); using var cmd=new MySqlCommand("SELECT stream_id FROM ecc_dof_wukrostmarycollege.streams ORDER BY stream_id",c); using var rd=cmd.ExecuteReader(); while(rd.Read()) r.Add(rd[0]?.ToString()??""); c.Close(); return r; });
                TxtStreamID.Items.Clear();
                foreach (var d in list) TxtStreamID.Items.Add(new System.Windows.Controls.ComboBoxItem{Content=d});
            } catch {}
        }
        ...
        private void Grid1_SelectionChanged(object s, SelectionChangedEventArgs e) {
            ...
            _selKey = r["level_id"]?.ToString() ?? "";
            TxtStreamID.Text = r["stream_id"]?.ToString() ?? "";
            TxtLevelID.Text = r["level_id"]?.ToString() ?? "";
            TxtLevel.Text = r["level"]?.ToString() ?? "";
            TxtOccName.Text = r["occupational_name"]?.ToString() ?? "";
            TxtNoCourses.Text = r["no_of_courses"]?.ToString() ?? "";
        }

        private async void BtnSave_Click(object s, RoutedEventArgs e) {
            if (string.IsNullOrWhiteSpace(TxtStreamID.Text) ||
                string.IsNullOrWhiteSpace(TxtLevelID.Text) ||
                string.IsNullOrWhiteSpace(TxtLevel.Text) ||
                string.IsNullOrWhiteSpace(TxtOccName.Text) ||
                string.IsNullOrWhiteSpace(TxtNoCourses.Text))
            { Msg("There is empty field(s). Please fill all fields!",false); return; }
            try {
                string stid=TxtStreamID.Text.Trim(), lid=TxtLevelID.Text.Trim(), lv=TxtLevel.Text.Trim(), occ=TxtOccName.Text.Trim(), nc=TxtNoCourses.Text.Trim();
                bool dup = await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("SELECT COUNT(*) FROM ecc_dof_wukrostmarycollege.levels WHERE level_id=@k",c); cmd.Parameters.AddWithValue("@k",lid); int n=Convert.ToInt32(cmd.ExecuteScalar()); c.Close(); return n>0; });
                if(dup){Msg("There is already a record with the same ID!",false);return;}
                await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("INSERT INTO ecc_dof_wukrostmarycollege.levels (stream_id,level_id,level,occupational_name,no_of_courses) VALUES(@s,@li,@lv,@o,@n)",c);
                    cmd.Parameters.AddWithValue("@s",stid); cmd.Parameters.AddWithValue("@li",lid);
                    cmd.Parameters.AddWithValue("@lv",lv); cmd.Parameters.AddWithValue("@o",occ);
                    cmd.Parameters.AddWithValue("@n",nc); cmd.ExecuteNonQuery(); c.Close(); });
        ...
        private async void BtnUpdate_Click(object s, RoutedEventArgs e) {
            ...
            if(TxtLevelID.Text.Trim()!=_selKey){Msg("Update attempt failed!",false);return;}
            try {
                string key=_selKey, stid=TxtStreamID.Text.Trim(), lv=TxtLevel.Text.Trim(), occ=TxtOccName.Text.Trim(), nc=TxtNoCourses.Text.Trim();
                await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("UPDATE ecc_dof_wukrostmarycollege.levels SET stream_id=@s,level=@lv,occupational_name=@o,no_of_courses=@n WHERE level_id=@key",c);
                    cmd.Parameters.AddWithValue("@s",stid); cmd.Parameters.AddWithValue("@lv",lv);
                    cmd.Parameters.AddWithValue("@o",occ); cmd.Parameters.AddWithValue("@n",nc);
        ...
        private async void BtnDelete_Click(object s, RoutedEventArgs e) {
            ...
            string key=_selKey;
            try {
                await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("DELETE FROM ecc_dof_wukrostmarycollege.levels WHERE level_id=@k",c); cmd.Parameters.AddWithValue("@k",key); cmd.ExecuteNonQuery(); c.Close(); });
                Msg("Delete successful!",true); await Load(Q); Clear();
            } catch(Exception ex){Msg("Connection failed! "+ex.Message,false);}
        }

        private async void TxtFilter_Changed(object s, TextChangedEventArgs e) { string t=TxtFilter.Text.Trim(); await Load(string.IsNullOrEmpty(t)?Q:Q+$" WHERE level_id LIKE '%{t}%' OR occupational_name LIKE '%{t}%'"); }
        ...
        private void Clear(){TxtStreamID.Text = TxtLevelID.Text = TxtLevel.Text = TxtOccName.Text = TxtNoCourses.Text="";_selKey="";}
        ...
        private async void BtnExportPdf_Click(object s, RoutedEventArgs e) => await ExportPdf(new[]{"stream_id","level_id","level","occupational_name","no_of_courses"}, new[]{"Stream ID","Level ID","Level","Occupational Name","No. Courses"}, "Levels");
        private async void BtnExportExcel_Click(object s, RoutedEventArgs e) => await ExportExcel(new[]{"stream_id","level_id","level","occupational_name","no_of_courses"}, new[]{"Stream ID","Level ID","Level","Occupational Name","No. Courses"}, "Levels");
        ...
        private async Task ExportPdf(string[] fields, string[] headers, string title)
        {
            ...
            try{
                ...
                await Task.Run(()=>{
                    ...
                    foreach(var _ in headers)tbl.AddColumn("3.5cm");
