...
namespace CollegeManagementWPF.Views
{
    public partial class LevelsPage : Page
    {
        ...
        public LevelsPage() { InitializeComponent(); ThemeManager.ThemeChanged += ApplyTheme; ApplyTheme(); Loaded += async (s,e) => await Load(Q); }
        ...
        private async void BtnSave_Click(object s, RoutedEventArgs e) {
            ...
            try {
                bool dup = await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("SELECT COUNT(*) FROM ecc_dof_wukrostmarycollege.levels WHERE level_id=@k",c); cmd.Parameters.AddWithValue("@k",TxtLevelID.Text.Trim()); int n=Convert.ToInt32(cmd.ExecuteScalar()); c.Close(); return n>0; });
                ...
                await Task.Run(() => { var c=_db.GetConnection(); c.Open(); var cmd=new MySqlCommand("INSERT INTO ecc_dof_wukrostmarycollege.levels (stream_id,level_id,level,occupational_name,no_of_courses) VALUES(@s,@li,@lv,@o,@n)",c);
                    ...
                    cmd.Parameters.AddWithValue("@lv",TxtLevel.Text.Trim());
                    cmd.Parameters.AddWithValue("@o",TxtOccName.Text.Trim());
                    cmd.Parameters.AddWithValue("@n",TxtNoCourses.Text.Trim());
        ...
        private async void TxtFilter_Changed(object s, TextChangedEventArgs e) { string t=TxtFilter.Text.Trim(); await Load(string.IsNullOrEmpty(t)?Q:Q+$" WHERE level_id LIKE '%{t}%'"); }
